...
const BuyScreen = (props) => {
  ...
  const getMarketDetails = useCallback(async () => {
    ...
    try {
      ...
      setTif(objectMarketDetails.data.market[3].tif);
    } catch (error) {
  ...
  const getMarketStatus = useCallback(async () => {
    try {
      ...
      setOrderStatus(objectMarketStatus.data.tradestatus);
    } catch (error) {
      throw error;
    }
  }, [dispatch]);
  ...
  const loadAllSecurities = useCallback(async () => {
    setIsLoading(true);
    await dispatch(loadingclientsActions.fetchClients());
    setIsLoading(false);
  }, [setIsLoading, dispatch]);

  const loadAllClients = useCallback(async () => {
    setIsLoading(true);
    await dispatch(dropdownsecuritiesActions.fetchDropDownAllSecurities());
    setIsLoading(false);
  ...
  useEffect(() => {
    ...
    getMarketDetails();
    getMarketStatus();
  }, [loadAllSecurities, loadAllClients, getMarketDetails, getMarketStatus]);
